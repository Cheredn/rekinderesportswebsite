    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            players = [
                ...
                (5, 'AIMIR666', 'Support', 'aimir666.png', 'main', 0),
                (6, 'LASQUA', 'OpenFragger', 'lasqua.png', 'main', 0),
...
async def cancel_handler(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("🚫 Действие отменено. Возврат в меню.", reply_markup=types.ReplyKeyboardRemove())
...
async def cmd_start(m: types.Message, state: FSMContext):
    ...
    kb.add("🗑️ Очистить игры", "🩺 Статус", "📥 Скачать БД")
    await m.answer("Rekinder Panel Ready.\nЕсли бот завис, напиши /cancel", reply_markup=kb)
...
async def p_call(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.message.answer("Пиши: НИК | РОЛЬ | КАПИТАН (1 или 0)\nПример: SHNYROQ | IGL | 1")
...
async def p_txt(m: types.Message, state: FSMContext):
    if '|' not in m.text:
        await m.answer("❌ Ошибка! Не забудь использовать | как разделитель.")
        return
    await state.update_data(t=m.text); await m.answer("Теперь скинь ФОТО игрока"); await States.p_p.set()
...
async def p_photo(m: types.Message, state: FSMContext):
    try:
        ...
        if len(parts) < 3: raise ValueError("Недостаточно данных")
        nick, role, cap = parts[0].strip(), parts[1].strip(), int(parts[2].strip())
        fn = f"{nick.lower()}.png"
        
        # Исправленный путь сохранения
        await m.photo[-1].download(destination_file=os.path.join(BASE_DIR, fn))
        ...
        await m.answer("❌ Ошибка! Проверь, что в конце текста стоит 1 или 0 (капитан).")
...
async def m_txt(m: types.Message, state: FSMContext): 
    if '|' not in m.text:
        ...
        return
    await state.update_data(t=m.text); await m.answer("ЛОГО противника"); await States.m_p.set()
...
async def m_photo(m: types.Message, state: FSMContext):
    try:
        ...
        await m.photo[-1].download(destination_file=os.path.join(OPPONENTS_DIR, fn))
        conn = sqlite3.connect(os.path.join(BASE_DIR, 'matches.db')); cursor = conn.cursor()
...
async def download_db(m: types.Message):
    db_path = os.path.join(BASE_DIR, 'matches.db')
    if os.path.exists(db_path):
        with open(db_path, 'rb') as file:
            await m.answer_document(file, caption="Актуальная база данных.")
    else: await m.answer("БД не найдена.")
