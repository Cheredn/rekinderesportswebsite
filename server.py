...
async def cmd_start(m: types.Message):
    ...
    kb.add("📥 Скачать БД") # НОВАЯ КНОПКА
    await m.answer("Управление Rekinder eSports", reply_markup=kb)

# Функция для получения файла базы данных
@dp.message_handler(lambda m: m.text == "📥 Скачать БД", user_id=ALLOWED_IDS)
async def download_db(m: types.Message):
    db_path = os.path.join(BASE_DIR, 'matches.db')
    if os.path.exists(db_path):
        with open(db_path, 'rb') as file:
            await m.answer_document(file, caption="Актуальный файл базы данных. Загрузите его на GitHub, чтобы сохранить игроков навсегда!")
    else:
        await m.answer("Файл базы данных не найден.")

# Остальная логика (изменение игрока, матчи) без изменений...
...
async def p_call(c: types.CallbackQuery, state: FSMContext):
    await state.update_data(pid=c.data.split('_')[1]); await c.message.answer("Пиши: НИК | РОЛЬ | КАПИТАН(1/0)"); await States.p_t.set()
...
async def p_photo(m: types.Message, state: FSMContext):
    try:
        ...
        fn = f"{nick.lower()}.png"; await m.photo[-1].download(os.path.join(BASE_DIR, fn))
        conn = sqlite3.connect(os.path.join(BASE_DIR, 'matches.db')); cursor = conn.cursor()
        cursor.execute("UPDATE players SET nickname=?, role=?, photo=?, is_captain=? WHERE id=?", (nick.upper(), role, fn, cap, data['pid']))
        conn.commit(); conn.close(); await m.answer("✅ Готово. Не забудь нажать 'Скачать БД' и обновить GitHub!"); await state.finish()
...
async def m_photo(m: types.Message, state: FSMContext):
    ...
    fn = f"logo_{opp.lower().replace(' ', '_')}.png"; await m.photo[-1].download(os.path.join(os.path.join(BASE_DIR, 'opponents'), fn))
    conn = sqlite3.connect(os.path.join(BASE_DIR, 'matches.db')); cursor = conn.cursor()
